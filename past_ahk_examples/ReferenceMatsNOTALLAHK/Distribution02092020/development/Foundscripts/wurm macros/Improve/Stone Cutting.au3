    ...
    While (1)
       if _IsPressed("01") And $setlockLoc==1 Then
		   ...
		   EndIf
			...
			if $timerr==1 Then
		   $length = InputBox("HOW LONG TO RUN", "How many minutes to run? *TOOLBELT ORDER*  2=SHARD, 3=chisel, *BIND* I Improve/Finish and R repair *Z Starts*", 30)
	 ...
	 EndIf
       ...
       if $running==1 Then
			...
			$getiron=0
			While $iron=1
			$iron = _ImageSearchArea("iron.bmp",1,$ulsearch[0],$ulsearch[1],$lrsearch[0],$lrsearch[1],$x1,$y1,50)
		   If $iron=1 Then
			If $getiron=0 Then
			GetIron()
			$getiron=1
		    ...
		    Local $hammer=1
			While $hammer=1
			$hammer = _ImageSearchArea("chisel.bmp",1,$ulsearch[0],$ulsearch[1],$lrsearch[0],$lrsearch[1],$x1,$y1,50)
		   If $hammer=1 Then
			Hammer()
			Repair()
			checktimer()
			...
			Local $whetstone=1
			While $whetstone=1
			$whetstone = _ImageSearchArea("rocks.bmp",1,$ulsearch[0],$ulsearch[1],$lrsearch[0],$lrsearch[1],$x1,$y1,50)
		   If $whetstone=1 Then
			Whetstone()
			Repair()
			checktimer()
