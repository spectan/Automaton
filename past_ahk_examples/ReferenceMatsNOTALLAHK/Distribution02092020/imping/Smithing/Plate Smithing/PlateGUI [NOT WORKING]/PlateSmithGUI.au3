#cs ==================================================================================================
	...
	P  L  A  T  E    S  M  I  T  H  E  R    G  U  I
	blinged out in rare plate
	~ Dissimulo scripts

	--------------------------------------------------------------------------------------------------
	Created on 13/08/2015
	Last Modified 13/08/2015
	Version 0.1 (Most useless tool edition)
	--------------------------------------------------------------------------------------------------
	...
	--------------------------------------------------------------------------------------------------
	Overview:
	Improves Plate Armour that is selectable from a GUI list
	--------------------------------------------------------------------------------------------------
...
Global $actions = 2 ;set to the amount of actions wanted to be queued (base on mind logic and stamina use)
...
Global $imagefound[14] ;Array for 'image', with x and y co-ordinate
...
#cs
	...
	- [10] lumpheat.bmp
	- [11] itemheat.bmp
	- [12] toolittlematerial.bmp
	- [13] toolittlewater.bmp
...
Func AnchorSet()
	Sleep(100)
	FindImage('Needleicontext.bmp', 0, 0, 0, @DesktopWidth, @DesktopHeight, $anchorXY, 3)
	If $imagefound[3] == 1 Then
		...
		Next
	Else
		MsgBox(0, "Debug Msgbox", "Needleicontext.bmp not found, exiting", 0, $handle) ;Debug
        Sleep(100)
        _Exit()
	EndIf
	FindImage('Needleicontext.bmp', 1, 0, 0, @DesktopWidth, @DesktopHeight, $anchorXY, 3)
	If $imagefound[3] == 1 Then
		For $n = 0 to $actions - 1
...
Func FindSmithingStoppers()
	Local $lumpheat, $itemheat, $toolittlematerial, $toolittlewater
	$lumpheat = _ImageSearchArea("lumpheat.bmp", 0, 0, 0, @DesktopWidth, @DesktopHeight, $x1, $y1, 30)
	$itemheat = _ImageSearchArea("itemheat.bmp", 0, 0, 0, @DesktopWidth, @DesktopHeight, $x1, $y1, 30)
	...
	If $lumpheat or $itemheat or $toolittlematerial or $toolittlewater = 1 Then
		$smithingstopper = 1
	Else
		$smithingstopper = 0
	EndIf
...
#Region ### START Koda GUI section ### Form=c:\users\tyrone\documents\wurm\development\guifrm.kxf
$guifrm = GUICreate("Gui Tester", 428, 353, 226, 158)
$Title = GUICtrlCreateLabel("PLATE SMITHER GUI", 80, 16, 288, 36)
...
$improveinp = GUICtrlCreateInput("i", 200, 200, 113, 21)
$repairinp = GUICtrlCreateInput("r", 200, 232, 113, 21)
$actionlbl = GUICtrlCreateLabel("Actions", 146, 104, 41, 17)
$improvelbl = GUICtrlCreateLabel("Improve bind", 120, 200, 65, 17)
$repairlbl = GUICtrlCreateLabel("Repair bind", 127, 232, 58, 17)
$soundlbl = GUICtrlCreateLabel("Don't play sound?", 97, 264, 86, 17)
$actioncmb = GUICtrlCreateCombo("Select", 200, 104, 113, 25)
GUICtrlSetData(-1, "Great Helm|Basinet Helm|Breast Plate|Leggings|Vambraces|Gauntlets|Sabatons")
$exitinp = GUICtrlCreateInput("Esc", 200, 168, 113, 21)
$exitlbl = GUICtrlCreateLabel("Exit bind", 141, 168, 49, 17)
$pauseinp = GUICtrlCreateInput(";", 200, 136, 113, 21)
$pauselbl = GUICtrlCreateLabel("Pause bind", 129, 137, 65, 17)
GUISetState(@SW_SHOW)
#EndRegion ### END Koda GUI section ###


;http://stackoverflow.com/questions/18891419/how-to-save-inputs-in-koda-for-designer
		...
		Case 1
			...
			If $stamina == 1 Then
				...
				If $smithingstopper = 0 Then
					...
					Resort(2)
					For $m = 0 To $actions - 1
						FindImage('hammericon.bmp', 1, $anchorimprove[0], $anchor[$m][1] + 16, $anchorimprove[1], $anchor[$m][1] + 16 + 17, $a, 4)
						Sleep(50)
						If $imagefound[4] == 1 Then
							Sleep(50)
							MouseMove($anchorcenter[$m][0] + 16 + Random(-4, 4), $anchorcenter[$m][1] + 16 + Random(-2, 2), 14 + Random(-1, 1))
							Sleep(_Random_Gaussian(200, 30, 1))
							Toolbelt(1)
							Sleep(_Random_Gaussian(200, 30, 1))
							Send($improvebind)
						...
						Else
							FindImage('whetstoneicon.bmp', 1, $anchorimprove[0], $anchor[$m][1] + 16, $anchorimprove[1], $anchor[$m][1] + 16 + 17, $a, 5)
							If $imagefound[5] == 1 Then
								...
								Sleep(_Random_Gaussian(160, 30, 1))
							Else
								FindImage('pelticon.bmp', 1, $anchorimprove[0], $anchor[$m][1] + 16, $anchorimprove[1], $anchor[$m][1] + 16 + 17, $a, 6)
								If $imagefound[6] == 1 Then
									...
									Sleep(_Random_Gaussian(160, 30, 1))
								Else
									FindImage('pelticon.bmp', 1, $anchorimprove[0], $anchor[$m][1] + 16, $anchorimprove[1], $anchor[$m][1] + 16 + 17, $a, 7)
									If $imagefound[7] == 1 Then
										...
										Sleep(_Random_Gaussian(160, 30, 1))
									Else
										FindImage('malleticon.bmp', 1, $anchorimprove[0], $anchor[$m][1] + 16, $anchorimprove[1], $anchor[$m][1] + 16 + 17, $a, 8)
