#cs ==================================================================================================
	...
	B  S  B    O  A  R    I  M  P  E  R
	freedomers love 90ql boats
	~ Dissimulo scripts
	...
	--------------------------------------------------------------------------------------------------
	Overview:
	Takes oars out of a BSB, imps the oars by using a carving knife, and puts them back in
	--------------------------------------------------------------------------------------------------
...
HotKeySet("{]}", "BSBSetLoc") ;Sets the BSB location for oar dragging
HotKeySet("{[}", "InvSetLoc") ;Sets the Inventory location for oar dragging
...
#cs
	...
	- [3] anchorimage.bmp
	- [4] damagetab.bmp
	- [5] oaricontextinv.bmp (anchor set)
	- [6] oaricontextbsb.bmp (oar in BSB)
...
Func AnchorSet()
	Sleep(100)
	FindImage('oaricontextinv.bmp', 0, 0, 0, @DesktopWidth, @DesktopHeight, $anchorXY, 5)
	If $imagefound[5] == 1 Then
		...
		Next
	Else
		MsgBox(0, "Debug Msgbox", "oaricontextinv.bmp not found, exiting", 0, $handle) ;Debug
        Sleep(100)
        _Exit()
	EndIf
	FindImage('oaricontextinv.bmp', 1, 0, 0, @DesktopWidth, @DesktopHeight, $anchorXY, 5)
	If $imagefound[5] == 1 Then
		For $n = 0 to $actions - 1
...
Func MainLoop()
	Switch $running
		Case 1
			FindStamina()
			If $stamina == 1 Then
				FindImage('oaricontextinv.bmp', 0, 0, 0, @DesktopWidth, @DesktopHeight, $a, 5)
				Sleep(50)
				If $imagefound[5] == 1 Then
					MouseMove($a[0] - 5 + Random(-2, 2), $a[1] + 4 + Random(-2, 2), 13 + Random(-1, 1))
					Sleep(_Random_Gaussian(200,30,1))
					...
					Equipped()
					AntiRand()
					FindImage('oaricontextinv.bmp', 1, 0, 0, @DesktopWidth, @DesktopHeight, $a, 5)
					If $imagefound[5] == 1 Then
				...
				ElseIf $imagefound[5] == 0 Then
					...
					If $imagefound[8] == 1 Then
						$running = 0
						MsgBox(0, "Debug Msgbox", "Didn't BSB oars, please reset", 0, $handle) ;Debug
					ElseIf $imagefound[8] == 0 Then
						FindImage('oaricontextbsb.bmp', 1, 0, 0, @DesktopWidth, @DesktopHeight, $a, 6)
						...
						ElseIf $imagefound[6] == 0 Then
							$running = 0
							MsgBox(0, "Debug Msgbox", "Couldn't find BSB oars", 0, $handle) ;Debug
						EndIf
					EndIf
				EndIf
			ElseIf $stamina == 0 Then
