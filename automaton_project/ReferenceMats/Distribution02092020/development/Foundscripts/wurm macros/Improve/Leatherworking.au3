    ...
    While (1)
       if _IsPressed("01") And $setlockLoc==1 Then
		   ...
		   EndIf
			...
			if $timerr==1 Then
		   $length = InputBox("HOW LONG TO RUN", "How many minutes to run? *TOOLBELT ORDER* 1=leather, 2=needle, 3=awl, 4=leatherknife, 5=mallet *BIND* I Improve/Finish and R repair *Z Starts*", 30)
	 ...
	 EndIf
       ...
       if $running==1 Then
			Local $iron=1
			While $iron=1
			$iron = _ImageSearchArea("pelt.bmp",1,$ulsearch[0],$ulsearch[1],$lrsearch[0],$lrsearch[1],$x1,$y1,50)
		   If $iron=1 Then
			Improveiron()
			Repair()
			checktimer()
		    ...
		    Local $hammer=1
			While $hammer=1
			$hammer = _ImageSearchArea("awl.bmp",1,$ulsearch[0],$ulsearch[1],$lrsearch[0],$lrsearch[1],$x1,$y1,50)
		   If $hammer=1 Then
			Hammer()
			Repair()
			checktimer()
			...
			Local $whetstone=1
			While $whetstone=1
			$whetstone = _ImageSearchArea("needle.bmp",1,$ulsearch[0],$ulsearch[1],$lrsearch[0],$lrsearch[1],$x1,$y1,50)
		   If $whetstone=1 Then
			Whetstone()
			Repair()
			checktimer()
			...
			Local $pelt=1
			While $pelt=1
			$pelt = _ImageSearchArea("knife.bmp",1,$ulsearch[0],$ulsearch[1],$lrsearch[0],$lrsearch[1],$x1,$y1,50)
		   If $pelt=1 Then
			Polish()
			Repair()
			checktimer()
			...
			Local $water=1
			While $water=1
			$water = _ImageSearchArea("mallet.bmp",1,$ulsearch[0],$ulsearch[1],$lrsearch[0],$lrsearch[1],$x1,$y1,50)
		   If $water=1 Then
			Temper()
			Repair()
			checktimer()
