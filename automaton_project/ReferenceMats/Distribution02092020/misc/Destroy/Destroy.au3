#cs ==================================================================================================
	...
	D  E  S  T  R  O  Y
	bash the steel mine doors
	~ Dissimulo scripts

	--------------------------------------------------------------------------------------------------
	Created on 7/01/2014
	Last Modified 9/01/2014
	Version 1.0 (hotkeys on my keychain)
	--------------------------------------------------------------------------------------------------
	...
	-0.8 (unneeded version update)
	--Fixed all internal stuff like 'BSB' location (hopefully)
	-0.9 (falls with a crash)
	--Added in detection for event mesage
	-1.0 (hotkeys on my keychain)
	--Moved destroy rclick action out of main loop to its own function, so it could be hotkeyed
...
HotKeySet("{]}", "BashSetLoc") ;Sets the location of the Object to be bashed
HotKeySet("{[}", "Destroy") ;Manual destroy hotkey
Global $actions = 5 ;set to the amount of actions wanted to be queued (base on mind logic and stamina use)
...
Func BashSetLoc()
	$BashLoc = MouseGetPos()
	$BashLocSet = 1
	TrayTip("", "Object location has been set", 5)
EndFunc   ;==>BashSetLoc

Func SetLocations()
	If $BashLocSet == 0 Then
		TrayTip("", "Set the object location (]) key", 5)
		Do
			Sleep(100)
		Until $BashLocSet == 1
	EndIf
...
Func Destroy()
	MouseMove($BashLoc[0] + Random(-7, 7), $BashLoc[1] + Random(-7, 7), 14 + Random(-1, 1))
	Sleep(_Random_Gaussian(80, 5, 1))
	MouseClick("right")
	Sleep(_Random_Gaussian(500, 30, 1))
	FindImage('destroytext.bmp', $menu0, 0)
	...
	ElseIf $imagefound[0] == 0 Then
		TrayTip("", "damagetext.bmp not found", 5)
		Sleep(100)
		$running = 0
	EndIf
EndFunc   ;==>Destroy
		...
		Case 1
			...
			If $stamina == 1 Then
				...
				ElseIf $imagefound[1] == 1 Then
					Sleep(50)
					TrayTip("", "Object destroyed, Macro stopped", 5)
					$running = 0
				EndIf
			ElseIf $stamina == 0 Then
				Sleep(500)
