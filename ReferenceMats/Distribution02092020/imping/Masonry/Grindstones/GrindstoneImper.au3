...
Global $actions = 3 ;set to the amount of actions wanted to be queued
;Chisel on toolbelt slot 1
;Rockshards on toolbelt slot 2
;-----------------------------------------------------------------------------------------------------
...
Func AnchorSet()
	Sleep(100)
	FindImage('grindstoneicontext.bmp', 0, 0, 0, @DesktopWidth, @DesktopHeight, $anchorXY, 0)
	If $imagefound[0] == 1 Then
		...
		Next
	Else
		MsgBox(0, "Debug Msgbox", "grindstoneicontext.bmp not found, exiting", 0, $handle) ;Debug
        Sleep(100)
        _Exit()
	EndIf
	FindImage('grindstoneicontext.bmp', 1, 0, 0, @DesktopWidth, @DesktopHeight, $anchorXY, 0)
	If $imagefound[0] == 1 Then
		For $n = 0 to $actions - 1
	...
	If $imagefound[6] == 1 Then
			...
			$anchordamage[1] = $anchorXY[0] + 38 ;x2
	Else
            ...
            If $stamina == 1 Then
				...
				For $m = 0 to $actions - 1
					...
					If $imagefound[3] == 1 Then
						Sleep(50)
						MouseMove($anchorcenter[$m][0] + Random(-2, 2), $anchorcenter[$m][1] + Random(-2, 2), 13 + Random(-1, 1))
