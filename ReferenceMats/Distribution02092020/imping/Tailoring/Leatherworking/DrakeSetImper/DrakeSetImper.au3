#cs ==================================================================================================
	...
	D R A K E  S E T  I M P E R
	because fuck epic
	~ Dissimulo scripts

	--------------------------------------------------------------------------------------------------
	Created on 6/09/2016
	Last Modified 6/09/2016
	Version 0.1 (Muscles done)
	--------------------------------------------------------------------------------------------------
	...
	--------------------------------------------------------------------------------------------------
	Overview:
	Improves a single set of drake (minus the cap because im not a fag)
	--------------------------------------------------------------------------------------------------
...
Global $imagefound[?] ;Array for 'image', with x and y co-ordinate
...
Func AnchorSet()
	Sleep(100)
	FindImage('anchorimage.bmp', 0, 0, 0, @DesktopWidth, @DesktopHeight, $anchorXY, 0)
	If $imagefound[0] == 1 Then
		...
		Next
	Else
		MsgBox(0, "Debug Msgbox", "anchorimage.bmp not found, exiting", 0, $handle) ;Debug
        Sleep(100)
        _Exit()
	EndIf
	FindImage('anchorimage.bmp', 1, 0, 0, @DesktopWidth, @DesktopHeight, $anchorXY, 0)
	If $imagefound[0] == 1 Then
		For $n = 0 to $actions - 1
	...
	Switch $running
		Case 1
			...
			If $stamina == 1 Then
				...
				Resort(2)
				For $m = 0 To $actions - 1
					FindImage('tool.bmp', 1, $anchorimprove[0], $anchor[$m][1], $anchorimprove[1], $anchor[$m][1] + 17, $a, ?)
					Sleep(50)
					If $imagefound[3] == 1 Then
						Sleep(50)
						MouseMove($anchorcenter[$m][0] + Random(-3, 3), $anchorcenter[$m][1] + Random(-2, 2), 13 + Random(-1, 1))
						Sleep(_Random_Gaussian(200, 30, 1))
						Toolbelt(1)
						Sleep(_Random_Gaussian(200, 30, 1))
						Send($improvebind)
