#cs ==================================================================================================
	...
	W  O  O  D  E  N    S  P  E  A  R    I  M  P  E  R
	Weaponsmithing without the forge
	~ Dissimulo scripts

	--------------------------------------------------------------------------------------------------
	Created on 3/12/2013
	Last Modified 3/12/2013
	Version 0.8 (Most useless weapon edition)
	--------------------------------------------------------------------------------------------------
...
Func AnchorSet()
	Sleep(100)
	FindImage('pinewoodlongspearicontext.bmp', 0, 0, 0, @DesktopWidth, @DesktopHeight, $anchorXY, 3)
	If $imagefound[3] == 1 Then
		...
		Next
	Else
		MsgBox(0, "Debug Msgbox", "pinewoodlongspearicontext.bmp not found, exiting", 0, $handle) ;Debug
        Sleep(100)
        _Exit()
	EndIf
	FindImage('pinewoodlongspearicontext.bmp', 1, 0, 0, @DesktopWidth, @DesktopHeight, $anchorXY, 3)
	If $imagefound[3] == 1 Then
		For $n = 0 to $actions - 1
	...
	Switch $running
		Case 1
			...
			If $stamina == 1 Then
				...
				Resort(2)
				For $m = 0 To $actions - 1
					FindImage('logicon.bmp', 1, $anchorimprove[0], $anchor[$m][1] + 16, $anchorimprove[1], $anchor[$m][1] + 33, $a, 4)
					Sleep(50)
					...
					Else
						FindImage('carvingknifeicon.bmp', 1, $anchorimprove[0], $anchor[$m][1] + 16, $anchorimprove[1], $anchor[$m][1] + 33, $a, 5)
						If $imagefound[5] == 1 Then
							...
							Sleep(_Random_Gaussian(160, 30, 1))
						Else
							FindImage('fileicon.bmp', 1, $anchorimprove[0], $anchor[$m][1] + 16, $anchorimprove[1], $anchor[$m][1] + 33, $a, 6)
							If $imagefound[6] == 1 Then
								...
								Sleep(_Random_Gaussian(160, 30, 1))
							Else
								FindImage('pelticon.bmp', 1, $anchorimprove[0], $anchor[$m][1] + 16, $anchorimprove[1], $anchor[$m][1] + 33, $a, 7)
								If $imagefound[7] == 1 Then
									...
									Sleep(_Random_Gaussian(160, 30, 1))
								Else
									FindImage('malleticon.bmp', 1, $anchorimprove[0], $anchor[$m][1] + 16, $anchorimprove[1], $anchor[$m][1] + 33, $a, 8)
