#cs ==================================================================================================
	...
	W  O  O  D  E  N    S  P  E  A  R    P  I  X  E  L    T  E  S  T
	imagesearch is fucked
	~ Dissimulo scripts

	--------------------------------------------------------------------------------------------------
	Created on 8/07/2015
	Last Modified 8/07/2015
	Version 0.1 (Most useless weapon edition)
	--------------------------------------------------------------------------------------------------
...
Global $actions = 10 ;set to the amount of actions wanted to be queued (base on mind logic and stamina use)
		...
		Case 1
			...
			If $stamina == 1 Then
				...
				For $m = 0 To $actions - 1
					...
					;$n = PixelGetColor($anchorimprove[0] + 7, $anchor[$m][1] + 5, $handle)
					;MouseMove($anchorimprove[0] + 7, $anchor[$m][1] + 5, 14 + Random(-1, 1))
					;Sleep(50)
					;ConsoleWrite("Position " & $m & ", Dec Color: " & $n & ", Hex Color: " & Hex($n, 6) & @CRLF)
					MouseMove($anchorimprove[0], $anchor[$m][1], 14 + Random(-1, 1))
					Sleep(_Random_Gaussian(200,30,1))
					MouseMove($anchorimprove[1], $anchor[$m][1] + 12, 14 + Random(-1, 1))
					Sleep(_Random_Gaussian(400,30,1))
				Next
				Sleep(10000)
			ElseIf $stamina == 0 Then
